...
public interface IFacilityRepository extends JpaRepository<Facility, Long> {
    @Query(value = "select * from facility where is_delete = false and name like concat('%',:nameSearch,'%') and facility_type_id like concat('%',:typeId,'%')", nativeQuery = true)
    Page<Facility> findAll(@Param("nameSearch") String nameSearch, @Param("typeId") String facilityType, Pageable pageable);
